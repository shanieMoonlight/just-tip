...
import { JtUiIcon } from './icon';

@Component({
  selector: 'jt-host',
  template: `<jt-ui-icon [theme]="theme" [iconSizePx]="size">test</jt-ui-icon>`,
  standalone: true,
  imports: [JtUiIcon],
})
...
describe('JtUiIcon (standalone)', () => {
  ...
  it('sets host class based on `theme` input', () => {
    ...
    const hostEl = fixture.nativeElement.querySelector('jt-ui-icon');
  ...
  it('sets CSS variable `--icon-size-px` from `iconSizePx` input', () => {
    ...
    const hostEl: HTMLElement = fixture.nativeElement.querySelector('jt-ui-icon');
    expect(hostEl).toBeTruthy();
  ...
  it('renders the inner <i> element and projects content', () => {
    ...
    const iconEl: HTMLElement | null = fixture.nativeElement.querySelector('jt-ui-icon i.material-icons');
