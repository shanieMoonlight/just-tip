 ...
 * an auth email is present in the `JustTipIoConfigService`.
 */
 ...
 *
 * The interceptor reads the auth value from the `JustTipIoConfigService` via
...
export const authEmailInterceptorFn: HttpInterceptorFn = (req, next) => {
    ...
    const authEmailStore = inject(JustTipIoConfigService)
